...
export const monthlyCommand = define({
	name: 'monthly',
	description: 'Show combined usage report grouped by month',
	args: {
		json: {
			type: 'boolean',
			short: 'j',
	...
	async run(ctx) {
		...
		const { data, totals } = await loadCombinedMonthlyData({
			sources,
			since,
			until,
		...
		if (data.length === 0) {
			log(jsonOutput ? JSON.stringify({ monthly: [], totals: null }) : 'No usage data found.');
			return;
		}
		...
		logger.box(`Omni Usage Report - Monthly (${formatSourcesTitle(sources)})`);

		const table: ResponsiveTable = new ResponsiveTable({
			head: ['Source', 'Month', 'Input', 'Output', 'Cache', 'Cost (USD)', 'Models'],
			colAligns: ['left', 'left', 'right', 'right', 'right', 'right', 'left'],
			compactHead: ['Source', 'Month', 'Input', 'Output', 'Cost (USD)'],
